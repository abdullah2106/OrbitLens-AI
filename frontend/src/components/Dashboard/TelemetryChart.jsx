...
function CustomTooltip({ active, payload, anomalyByTs, field }) {
  ...
  return (
    ...
    >
      <p style={{ margin: '0 0 4px', color: '#57606a' }}>{ts}</p>
      <p style={{ margin: '0 0 4px', color: '#1f2328', fontWeight: 600 }}>
        {field}: {typeof value === 'number' ? value.toFixed(3) : value}
      </p>
      {anomaly && (
        <>
          <p style={{ margin: '4px 0 2px', color: SEVERITY_COLORS[anomaly.severity] ?? '#57606a', fontWeight: 600 }}>
            Anomaly -- severity: {anomaly.severity}
          </p>
          <p style={{ margin: 0, color: '#57606a', lineHeight: 1.5 }}>
...
export default function TelemetryChart({ field, rows, anomalies }) {
  ...
  return (
    <div>
      <h4
        className="text-xs font-semibold text-[#57606a] mb-1 uppercase tracking-wide"
      >
        {field.replace(/_/g, ' ')}
      </h4>
        ...
        <LineChart data={rows} margin={{ top: 8, right: 12, bottom: 4, left: 0 }}>
          ...
          {anomalies.map((anomaly) => (
            <ReferenceDot
              ...
              y={anomaly.value}
              r={5}
              fill={SEVERITY_COLORS[anomaly.severity] ?? '#57606a'}
              stroke="#fff"
              strokeWidth={1.5}
            >
              <Label
                ...
                position="top"
                style={{ fontSize: 9, fill: SEVERITY_COLORS[anomaly.severity] ?? '#57606a', fontWeight: 700 }}
              />
            </ReferenceDot>
          ))}
        </LineChart>
