...
def _build_response(df: pd.DataFrame) -> dict:
    ...
    # -- Session -----------------------------------------------------------------
    # health_score is stored on the session here -- this is the SINGLE place it
    # is computed. report_generator.py reads it back rather than recomputing it,
    # to avoid the split-brain bug where two independent formulas disagree.
