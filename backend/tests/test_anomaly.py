...
Assertions:
  ...
  4. Total detected anomaly count stays within a sane bound of ground truth --
     guards against the statistical detector regressing to flagging noise
     across the whole dataset (the original bug this test suite failed to
     catch: 215 flags on a 28-row ground truth, with all other tests here
     still passing).
"""
...
def test_total_anomaly_count_within_sane_bound():
    """
    Assert total detected anomaly count stays within a sane bound relative to
    ground truth (28 injected rows across 3 windows).

    Regression guard: the original statistical detector flagged 215 anomalies
    on this same dataset -- a 7x-plus inflation over ground truth, caused by
    triggering on rolling z-score alone rather than requiring the value to be
    outside the field's nominal range first. None of the other tests in this
    file would have caught that regression, since they only check ground-truth
    coverage and high-severity precision, not overall volume.

    Bound: allow up to 2x the ground-truth row count (56) to leave room for
    isolation forest's independent, non-zero false-positive rate (see
    CONTAMINATION_ESTIMATE in isolation_forest.py) without being so loose that
    a real regression back toward 215 would pass silently.
    """
    df = _load_sample()
    gt_entries = _load_ground_truth()
    ...
    gt_row_count = sum(e["row_end"] - e["row_start"] + 1 for e in gt_entries)
    bound = gt_row_count * 2

    assert len(all_anomalies) <= bound, (
        f"Detected {len(all_anomalies)} anomalies, expected <= {bound} "
        f"(2x ground truth of {gt_row_count} rows). This may indicate the "
        f"statistical or isolation forest detector is flagging noise -- see "
        f"the regression this test guards against in the module docstring."
    )
